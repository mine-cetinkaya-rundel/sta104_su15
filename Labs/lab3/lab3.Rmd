---
title: "Lab 3: Foundations for statistical inference"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
download("https://stat.duke.edu/courses/Summer15/sta104.01-1/post/labs/sta104_lab3.Rmd", destfile = "sta104_lab3.Rmd")
```
</div>
...
distribution, plot all three distributions.
...
6. Take a random sample of size 50 from `price`. Using this sample, what is 
    your best point estimate of the population mean?

7. Since you have access to the population, simulate the sampling 
...
8. Change your sample size from 50 to 150, then compute the sampling 
...
9. Of the sampling distributions from 2 and 3, which has a smaller spread?  If
...
In Part 2 of the lab we'll start with a simple random sample of size 60 from the 
population. Specifically, this is a simple random sample of size 60. Note that 
the data set has information on many housing variables, but for now we'll just focus on the `area`.

```{r sample, eval=FALSE}
samp <- sample(area, 60)
```

10.  Describe the distribution of your sample. What would you say is the 
"typical" size within your sample? Also state precisely what you interpreted 
"typical" to mean.

11.  Would you expect another team's distribution to be identical to yours? 
...
12.  For the confidence interval to be valid, the sample mean must be normally 
distributed and have standard error $s / \sqrt{n}$. What conditions must be 
met for this to be true?

### Confidence levels

13.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.
...
14.  Does your confidence interval capture the true average size of houses in 
Ames? If you are working on this lab in a classroom, does your neighbor's 
interval capture this value? 

15.  Each student in your class should have gotten a slightly different 
...
16.  Using the following function (which was downloaded with the data set), 
...
17.   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?

18.   Calculate 50 confidence intervals at the confidence level you chose in the 
...
19.   What concepts from the textbook are covered in this lab?  What concepts, 
