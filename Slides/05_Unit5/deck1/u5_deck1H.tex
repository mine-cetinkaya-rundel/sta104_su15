...
\item No OH tomorrow (Tuesday) but I will be answering questions on Piazza throughout the day + can stay for a bit after class for quick questions

\item PS 3 feedback:
\begin{itemize}
\item 4.12 (c): 95\% of such random samples would have a sample mean between \$80.31 and \$89.11. \\
CIs are not about future sample statistics, they're about the unknown population parameter
\item Calculating required sample size for a target ME: \\
Be careful with your math + always round up, i.e. if the minimum required sample size is calculated to be 35.12 people, we would say we need at least 36 people even though 35.12 should round down to 35 because $n = 35$ is not sufficient.
\end{itemize}
...
\activity{$\pi$}{Write out the digits of $\pi$ from memory in the chat box. No cheating!}
